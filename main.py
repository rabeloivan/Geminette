...
if __name__ == "__main__":
	...
	if test_map is None:
		error_msg = f"Current directory '{dir_name}' does not match expected pattern (C00~13)."
	...
	console.print(f"[sea_green2]{'Generating test for ' + display_name + '...':^75s}[/sea_green2]", highlight=False)
	print()
	...
	for student_file_path in sorted_paths:
		...
		if run_func is None:
			...
			console.print(f"[yellow]Test for {student_file_path} not implemented yet.[/yellow]")
		...
		if not check_norminette(student_file_path):
			results.append((ex_name, "NORM ERR"))
			continue
	
		passed = run_func(student_file_path)

		results.append((ex_name, "OK" if passed else "KO"))
