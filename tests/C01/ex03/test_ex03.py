...
def run_C01_ex03(student_file):
    ...
    for i, (a, b, expected) in enumerate(test_cases, 1):
        ...
        if actual == expected:
            print_test_pass(
                i,
                f"{desc} set div to {disp_act_div} and mod to {disp_act_mod} as expected",
            )
            passed_count += 1
        else:
            print_test_fail(
                i,
                f"{desc} set div to {disp_act_div} and mod to {disp_act_mod} but expected div = {disp_exp_div} and mod = {disp_exp_mod}",
            )
