...
def run_C01_ex04(student_file):
    ...
    for i, (a, b, expected) in enumerate(test_cases, 1):
        ...
        if actual == expected:
            print_test_pass(
                i, f"{desc} set a to {disp_act_div} and b to {disp_act_mod} as expected"
            )
            passed_count += 1
        else:
            print_test_fail(
                i,
                f"{desc} set a to {disp_act_div} and b to {disp_act_mod} but expected a = {disp_exp_div} and b = {disp_exp_mod}",
            )
