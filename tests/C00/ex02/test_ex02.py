...
def run_C00_ex02(student_file):
	...
	if actual == expected:
		print_test_pass(1, f"ft_print_reverse_alphabet() output {disp_act} as expected")
		passed = True
	else:
		print_test_fail(1, f"ft_print_reverse_alphabet() output {disp_act} but expected {disp_exp}")
